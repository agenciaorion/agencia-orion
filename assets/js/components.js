...
function renderNavbar() {
  ...
  const mobileLinks = links
    .map(({ href, label }) => {
      const isActive = currentPage === href ? "text-primary font-bold border-l-4 border-primary bg-primary/5" : "text-on-surface border-l-4 border-transparent";
      return `
        <a href="${href}"
           class="block py-4 px-5 font-label-bold text-base uppercase tracking-wider ${isActive} hover:text-primary hover:border-primary hover:bg-primary/5 rounded-r-xl transition-all">
          ${label}
        </a>`;
    })
    .join("");
    ...
    <nav id="main-nav" class="bg-surface/90 backdrop-blur-md fixed top-0 left-0 right-0 z-50 border-b border-outline-variant/30 transition-all duration-300">
      ...
      <div id="mobile-menu" class="md:hidden fixed inset-0 top-[72px] z-[300] px-6 py-8 flex flex-col gap-2 border-t border-outline-variant/30" style="background:#fcf9f8;">
        ...
        <div class="mt-auto pt-8 border-t border-outline-variant/20">
          <a href="${WHATSAPP_LINK}" target="_blank"
             class="flex items-center justify-center gap-3 bg-primary text-on-primary font-label-bold px-6 py-4 rounded-xl text-sm uppercase tracking-wider">
            <svg fill="currentColor" width="20" height="20" viewBox="0 0 16 16"><path d="M13.601 2.326A7.854 7.854 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.933 7.933 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.898 7.898 0 0 0 13.6 2.326zM7.994 14.521a6.573 6.573 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.557 6.557 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592zm3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.133.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.1-.114.133-.198.198-.33.065-.134.034-.248-.015-.347-.05-.099-.445-1.076-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.729.729 0 0 0-.529.247c-.182.198-.691.677-.691 1.654 0 .977.71 1.916.81 2.049.098.133 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.058 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232z"/></svg>
            Falar com a Orion
          </a>
        </div>
      </div>
